    ...
    >
      ...
      >
        ...
        <p style={{ color: "#666", fontSize: 14, margin: "0 0 20px", lineHeight: 1.6 }}>
          Are you sure you want to reset the canvas?<br />You won't be able to undo this.
        </p>
        <div className="modal-actions">
    ...
    >
      ...
      >
        ...
        <div className="modal-actions" style={{ flexDirection: "column", gap: 10 }}>
          <ABtn onClick={onSaveAndContinue} className="btn-primary">💾 Save &amp; Continue</ABtn>
          <ABtn onClick={onDiscard} className="btn-danger">Discard Changes</ABtn>
          <ABtn onClick={onCancel}>Cancel</ABtn>
        </div>
      </motion.div>
    </motion.div>
    ...
    <div className="editor">
      <div className="sidebar">
        ...
        <div className="palette palette--wide">
          {palette.map(c => (
            <motion.button
              ...
              style={{ background: c, boxShadow: color === c ? "0 0 0 3px #D75050" : "0 0 0 1px #aaa" }}
        ...
        <motion.button
          onClick={handleSave}
          className="btn-save"
          whileHover={{ scale: 1.04, y: -2, boxShadow: "0 8px 24px rgba(215,80,80,0.45)" }}
          whileTap={{ scale: 0.97 }}
          transition={{ type: "spring", stiffness: 380, damping: 18 }}
          style={{ marginTop: 16 }}
        >
      ...
      {/* Clear confirmation */}
      <AnimatePresence>
        {showClearModal && <ClearModal onConfirm={handleClear} onCancel={() => setShowClearModal(false)} />}
      </AnimatePresence>
...
function Lookahead({ sections, sectionCursor, cursor, total, granularity, current }) {
  ...
  // Section mode: show only the next row
  const upcomingRows = [];
  const seen = new Set();
  for (const s of sections.slice(sectionCursor + 1)) {
    ...
    if (upcomingRows.length >= 1) break;
  }
    ...
    const applyHighlight = () => {
      ...
      if (granularity === "row") {
        ...
        pixels = rowSecs.flatMap(s => s.cols.map(c => ({
          rowIndex: s.rowIndex, columnIndex: c, color: "#D75050",
        })));
      } else {
        pixels = current.cols.map(c => ({
          rowIndex: current.rowIndex, columnIndex: c, color: "#D75050",
        }));
      }
